...
1️⃣ **客戶端** 發送 `SYN`（同步請求）到伺服器  
2️⃣ **伺服器** 回應 `SYN-ACK`（確認請求）  
3️⃣ **客戶端** 再回應 `ACK`，連線建立成功 ✅
...
`GET / HTTP/1.1 Host: www.google.com User-Agent: Chrome/114.0 Accept: text/html`

🔹 **過程**： 1️⃣ **HTTPS 握手**（TLS/SSL 加密）  
2️⃣ **瀏覽器傳送 HTTP `GET` 請求**（請求 HTML、CSS、JS 等資源）  
3️⃣ **伺服器回應 HTTP `200 OK`**，並傳回 HTML 內容
...
- 解析標籤：
    ...
    `<html>   <head>     <title>Google</title>   </head>   <body>     <h1>Hello World</h1>   </body> </html>`
...
**輸入網址後，發生了這些事：** 1️⃣ **DNS 查詢**（將網址解析為 IP）  
2️⃣ **TCP 連線（3 次握手）**  
3️⃣ **HTTPS 請求（加密傳輸）**  
4️⃣ **伺服器處理請求 & 回應 HTML**  
5️⃣ **瀏覽器解析 HTML、CSS、JS，構建 DOM & Render Tree**  
6️⃣ **Layout & Paint（排版 & 繪製）**  
7️⃣ **合成 & 顯示（GPU 合成畫面）**  
8️⃣ **使用者互動 & 瀏覽器回應**
